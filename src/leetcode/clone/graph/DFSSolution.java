...
public class DFSSolution
{
	...
	{
		return cloneDFS(node, new HashMap<Integer, UndirectedGraphNode>());
	}
			...
			Map<Integer, UndirectedGraphNode> newNodes)
	{
		...
		newNodes.put(Integer.valueOf(node.label), newNode);

		for (UndirectedGraphNode child : node.neighbors)
		{
			if (newNodes.containsKey(Integer.valueOf(child.label)))
			{
				newNode.neighbors.add(newNodes.get(child.label));
			}
			else
			{
				newNode.neighbors.add(cloneDFS(child, newNodes));
			}
		}
